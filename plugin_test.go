...
func TestPluginName(t *testing.T) {
	require.Equal(t, "beanstalk", (&Plugin{}).Name())
}

// TestPluginInit covers the enable rule: the driver needs the global section
// holding the beanstalkd address, and disables itself without it.
func TestPluginInit(t *testing.T) {
	require.NoError(t, (&Plugin{}).Init(&testLogger{}, newCfg("beanstalk")))
